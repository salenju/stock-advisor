...
subgraph "数据"
File["data/holdings.json<br/>持仓与交易记录"]
end
UI --> Composable
Composable --> Server
Server --> Store
...
## 架构总览
系统通过前端发起交易请求，后端路由解析并执行交易逻辑，更新 holdings.json，并在读取时进行数据迁移与派生字段计算。
...
## 结论
Stock Advisor 的数据模型以 Holding 为核心，通过 purchases[] 与 sells[] 解耦买入与卖出行为，结合 LIFO 成本结转与加权平均算法，确保成本与盈亏计算的准确性与可追溯性。数据迁移机制保障了历史数据的平滑升级，严格的验证规则与幂等导入提升了系统的健壮性。前端通过统一的 composable 封装 API，简化了交互复杂度，提升了开发效率。
