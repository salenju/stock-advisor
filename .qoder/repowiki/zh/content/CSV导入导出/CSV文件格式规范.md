...
## 更新摘要
**变更内容**
- 更新了默认CSV文件名格式：从`买入-卖出记录 -sheet1.csv`调整为`买入-卖出记录 - Sheet1.csv`（增加了空格）
- 更新了所有相关文件路径引用和示例模板中的文件名
- 保持了文档整体结构和功能描述的一致性
...
- **默认文件路径**：`data/买入-卖出记录 - Sheet1.csv`（注意：文件名中包含空格）
- 表头（顺序可调整，但需包含必需列）：
...
- **重要**：默认CSV文件名为`买入-卖出记录 - Sheet1.csv`，注意连字符后的空格
...
- **文件路径错误**：确保使用正确的默认文件名`买入-卖出记录 - Sheet1.csv`（注意空格）
...
- **默认文件找不到**：确认CSV文件名为`买入-卖出记录 - Sheet1.csv`（注意空格），或指定正确的文件路径
...
## 结论
本规范明确了 Stock Advisor 的 CSV 导入格式、字段要求、数据格式、匹配算法与错误处理机制。遵循本规范可确保跨市场（A 股、港股、美股）的买卖记录稳定导入，并保持持仓数据的准确性与一致性。特别注意默认CSV文件名的格式变更，确保使用正确的文件名`买入-卖出记录 - Sheet1.csv`。
